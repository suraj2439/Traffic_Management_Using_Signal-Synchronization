...
# Ensure that these import statements are removed/changed in the final project. Because this won't be the main file. The main function should call initialise_vehicles
# This file only has the code, and I couldn't run it without the rest of the bits. Try calling the initialise_vehicles function in your main file.
...
class Vehicle:
	...
	# Returns 1 if attribute is not already defined -- For coding purposes only. REMOVE FROM FINAL PROJECT.
...
'''X = (0, 50, 82, 110, 115, 143, 175, 262, 300, 335, 458, 495, 528, 690, 730, 765, 980, 1012, 1042, 1072, 1100)
Y = (0, 205, 233, 260, 285, 315, 495, 528, 565, 623, 660, 695)'''



'''
Changing the meaning of lane number. The tuple road_lanes_dimensions below contains the details of the dimensions for each lane of each road.
...
def choose_position(road_no, lane_no, lane_specific_dimensions, lane_common_dimensions, length, width):
	...
	while(i < len(vehicles) and change_count <= NO_OF_VEH): 
...
def initialise_vehicles():
	...
	for i in range(NO_OF_VEH):
		...
		while (position_x, position_y) == (CHOOSE_POSITION_FAILED) and attempt_count < NO_OF_VEH	:
